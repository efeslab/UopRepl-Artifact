...
def main():
    ...
    lru_config= open("6_8_512_lru_0_0_no.config","r").read()
    ghrp_config=open("6_8_512_ghrp_0_0_no.config","r").read()
    ...
    for app_idx, app in enumerate(appList):
        ...
        for i in range(len(chosen)):
            ...
            genOut.write(sh3.format(50, app)) # For self / cross
            genOut.close()
            ...
            genOut=open(configFolder/"{}_ghrp_{}.config".format(app, i),"w")
            genOut.write(ghrp_config.format(chosen[i],inst_count))
            genOut.close()
