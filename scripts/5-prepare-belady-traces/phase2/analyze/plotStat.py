...
def sortColumns(df):
    orderList = ["lru", "SRRIP", "SHiP++", "Mockingjay", "GHRP", "FURBYS"]
    df = df[orderList]
    return df
...
redirect = data.pivot(index='app', columns='Para3', values='Wasteful_Cycles')
redirect = sortColumns(redirect)
for col in redirect.columns.values:
    if col == 'lru':
        continue
...
plot_bar_chart(output_path=basePath/"Wasteful_Cycles.png",
               df=redirect,
               ytitle="Wasteful Cycles (\%)",
               xlabel_rotation=0,
               legend_col_num=10,
               show_legend=True,
               plot_type="bar",
