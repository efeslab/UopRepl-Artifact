...
/* Value for Power */
std::map<std::string, uint64_t> powerMap;
std::string FILE_TAG;
...
int DECODE_CYCLES = 5;
int MAP_CYCLES = 5;
int NUM_FUS = 8; // FU_TYPES

int ISSUE_WIDTH = 6;
int IC_FETCH_WIDTH = 4;
int UC_FETCH_WIDTH = 8;

int RS_FILL_WIDTH = 0;
int NODE_RET_WIDTH = 8;
int POWER_NUM_FPUS = 6; // From template xml
int CFS_PER_CYCLE = 6;
int POWER_NUM_ALUS = 4;
int POWER_NUM_MULS_AND_DIVS = 2;
int POWER_TOTAL_INT_RS_SIZE = 96;
int NODE_TABLE_SIZE = 256;
int POWER_TOTAL_FP_RS_SIZE = 64;
int DCACHE_READ_PORTS = 2;
int CRS_ENTRIES = 32;

/* Icache */
int ICACHE_SIZE = 32768;
int ICACHE_LINE_SIZE = 64;
int ICACHE_ASSOC = 8;
int ICACHE_BANKS = 1;

/* Dcache */
int DCACHE_SIZE = 32768;
int DCACHE_LINE_SIZE = 64;
int DCACHE_ASSOC = 8;
int DCACHE_BANKS = 2;
int DCACHE_CYCLES = 2;
...
/* Memory Control */
int RAMULATOR_TCK = 833333;
int BUS_WIDTH_IN_BYTES = 8;
int RAMULATOR_CHANNELS = 1;
int RAMULATOR_RANKS = 1;
int RAMULATOR_READQ_ENTRIES = 32;
int RAMULATOR_WRITEQ_ENTRIES = 32;
int DRAM_TECH_IN_NM = 32;
int RAMULATOR_TBL = 4;

uint POWER_INTF_REF_CHIP_FREQ = 3.2e9;
...
void power_print_system_params(std::ofstream& out) {
  ...
  ADD_XML_PARAM(out, header, "core_tech_node", 22, "nm");  // 22nm default
   ...
   * any computation */
  ADD_XML_CORE_STAT(out, header, 0, "total_cycles", powerMap["POWER_CYCLE"], );
  ADD_XML_STAT(out, header, "idle_cycles", 0, "Scarab: McPAT ignores this");
  ADD_XML_CORE_STAT(out, header, 0, "busy_cycles", powerMap["POWER_CYCLE"],
...
void power_print_core_params(std::ofstream& out, uint32_t core_id) {
  ...
  double ops_per_cycle = (powerMap["POWER_CYCLE"]) ?
                           ((double)powerMap["POWER_OP"]) /
                             powerMap["POWER_CYCLE"] :
  ...
  assert(OPC_TO_PEAK_OPC_RATIO <= 1);
  ...
  /**** Like Xeon ****/
  ADD_XML_PARAM(out, header, "vdd", 1.25, );
  ADD_XML_PARAM(out, header, "power_gating_vcc", -1, );
  /*****/
  ADD_XML_PARAM(
   ...
   * BTB once.*/
  ADD_XML_PARAM(
    out, header, "fetch_width", IC_FETCH_WIDTH,
  ...
  /* Registers */
  ADD_XML_PARAM(out, header, "archi_Regs_IRF_size", 192,
                "Number of integer architectural registers");
  ADD_XML_PARAM(out, header, "archi_Regs_FRF_size", 0,
  ...
  ADD_XML_PARAM(
    out, header, "phy_Regs_IRF_size", 192,
  ...
  ADD_XML_PARAM(out, header, "phy_Regs_FRF_size", 160,
  ...
  ADD_XML_CORE_STAT(out, header, core_id, "total_instructions", powerMap["POWER_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "uop_miss", powerMap["UOP_CACHE_MISS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "uop_hit", powerMap["UOP_CACHE_HIT"], );
  // POWER_UOPCACHE_ACCESS
  ADD_XML_CORE_STAT(out, header, core_id, "uop_cache_access", powerMap["POWER_UOPCACHE_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "uop_cache_miss", powerMap["POWER_UOPCACHE_MISS"], );

  ADD_XML_CORE_STAT(out, header, core_id, "int_instructions", powerMap["POWER_INT_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "fp_instructions", powerMap["POWER_FP_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "branch_instructions",
                    powerMap["POWER_BRANCH_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "branch_mispredictions",
                    powerMap["POWER_BRANCH_MISPREDICT"], );
  ADD_XML_CORE_STAT(out, header, core_id, "load_instructions", powerMap["POWER_LD_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "store_instructions", powerMap["POWER_ST_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "committed_instructions",
                    powerMap["POWER_COMMITTED_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "committed_int_instructions",
                    powerMap["POWER_COMMITTED_INT_OP"], );
  ADD_XML_CORE_STAT(out, header, core_id, "committed_fp_instructions",
                    powerMap["POWER_COMMITTED_FP_OP"], );
  ...
  ADD_XML_CORE_STAT(out, header, core_id, "total_cycles", powerMap["POWER_CYCLE"], );
  ADD_XML_STAT(out, header, "idle_cycles", 0, );
  ADD_XML_CORE_STAT(out, header, core_id, "busy_cycles",powerMap["POWER_CYCLE"], );
  ...
  ADD_XML_CORE_STAT(out, header, core_id, "ialu_accesses", powerMap["POWER_IALU_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "fpu_accesses", powerMap["POWER_FPU_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "mul_accesses", powerMap["POWER_MUL_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "cdb_alu_accesses",
                    powerMap["POWER_CDB_IALU_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "cdb_mul_accesses",
                    powerMap["POWER_CDB_MUL_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "cdb_fpu_accesses",
                    powerMap["POWER_CDB_FPU_ACCESS"], );
  ...
  ADD_XML_CORE_STAT(out, header, core_id, "read_accesses",
                    powerMap["POWER_ICACHE_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, core_id, "read_misses", powerMap["POWER_ICACHE_MISS"], );
  /* Note: conflicts parameter is not used in McPat anywhere, although some of
   * the predefined descriptor files have non-zero values. */
  ADD_XML_STAT(out, header, "conflicts", 0, );
  ...
  ADD_XML_CORE_STAT(out, header, core_id, "read_accesses", powerMap["POWER_BTB_READ"],
                    "See IFU code for guideline");
  ADD_XML_CORE_STAT(out, header, core_id, "write_accesses", powerMap["POWER_BTB_WRITE"], );
...
void power_print_l2_params(std::ofstream& out, uint32_t l2_id) {
  ...
  ADD_XML_CORE_STAT(out, header, l2_id, "read_accesses",
                    powerMap["POWER_LLC_READ_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, l2_id, "write_accesses",
                    powerMap["POWER_LLC_WRITE_ACCESS"], );
  ADD_XML_CORE_STAT(out, header, l2_id, "read_misses", powerMap["POWER_LLC_READ_MISS"], );
  ADD_XML_CORE_STAT(out, header, l2_id, "write_misses", powerMap["POWER_LLC_WRITE_MISS"], );
  /* Note: conflicts parameter is not used in McPat anywhere, although some of
   * the predefined descriptor files have non-zero values. */
  ADD_XML_STAT(out, header, "conflicts", 0, );
...
void power_print_l3_params(std::ofstream& out) {
  ...
  ADD_XML_ACCUM_STAT(out, header, "read_accesses", powerMap["POWER_LLC_READ_ACCESS"], );
  ADD_XML_ACCUM_STAT(out, header, "write_accesses", powerMap["POWER_LLC_WRITE_ACCESS"], );
  ADD_XML_ACCUM_STAT(out, header, "read_misses", powerMap["POWER_LLC_READ_MISS"], );
  ADD_XML_ACCUM_STAT(out, header, "write_misses", powerMap["POWER_LLC_WRITE_MISS"], );
  /* Note: conflicts parameter is not used in McPat anywhere, although some of
   * the predefined descriptor files have non-zero values. */
  ADD_XML_STAT(out, header, "conflicts", 0, );
  ADD_XML_STAT(out, header, "duty_cycle", 1, );
...
void power_print_mc_params(std::ofstream& out) {
  ...
  ADD_XML_ACCUM_STAT(out, header, "memory_accesses",
                     powerMap["POWER_MEMORY_CTRL_ACCESS"], );
  ADD_XML_ACCUM_STAT(out, header, "memory_reads", powerMap["POWER_MEMORY_CTRL_READ"], );
  ADD_XML_ACCUM_STAT(out, header, "memory_writes", powerMap["POWER_MEMORY_CTRL_WRITE"], );
...
void power_print_memory_parts(std::ofstream& out) {
  ...
  uint64_t CHIP_SIZE_IN_BYTES = uint64_t(8192) * 1024 *
                                1024 / 8;  // Convert MBits to Bytes (8192 from directly print ramulator.get_chip_size())
  assert(CHIP_SIZE_IN_BYTES != 0 && CHIP_SIZE_IN_BYTES <= (1 << 30));
  ...
  uint64_t DRAM_CHIP_ROW_BUFFER_SIZE = 4096;
...
int main(int argc, char** argv) {
  if(argc != 2) {
    std::cout << "Usage: " << argv[0] << " <working dir>" << std::endl;
    exit(1);
  }
  /* Read power.stat.0.out */
  FILE_TAG = std::string(argv[1]) + "/";
  std::string power_trace_file_name = FILE_TAG + "power.stat.0.out";
  std::ifstream file(power_trace_file_name);
  if(!file.good()) {
    std::cout << "Error: Could not open file " << power_trace_file_name
              << std::endl;
    exit(1);
  }
  std::string line;
  while (std::getline(file, line)) {
    if (line.find("Cumulative:") != std::string::npos) {
      break;
    }
  }
  while (std::getline(file, line)) {
  ...
  /* Read memory.stat.0.out */
  std::string memory_trace_file_name = FILE_TAG + "memory.stat.0.out";
  std::ifstream file2(memory_trace_file_name);
  if(!file2.good()) {
    std::cout << "Error: Could not open file " << memory_trace_file_name
              << std::endl;
    exit(1);
  }
  while (std::getline(file2, line)) {
    ...
    std::istringstream iss(line);
    iss >> key >> value;
    if(key != "UOP_CACHE_MISS" && key != "UOP_CACHE_HIT") continue;
    powerMap[key] = value;
    if(key == "UOP_CACHE_HIT") break;
  }
