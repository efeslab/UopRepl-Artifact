...
map<string, int> second_overall_execution_count;
...
void read_init() {
  if (FOOTPRINT) {
    ...
    // for(auto it = cl_decoded_entries.begin(); it != cl_decoded_entries.end(); ++it)
    // {
...
void read_init_cacheaccess_granularities() {
  if (FOOTPRINT) {
    ...
    for(uint64_t j = 0; j < all_strings.size(); j++) {
      ...
      // std::cout<<function_start<<' '<<target<<std::endl;
      second_overall_execution_count[addr]=(count);
    }
    // count_sort(overall_execution_count); // sort to ordered_overall_execution_count
    ordered_overall_execution_count = sorta(second_overall_execution_count); 
    uns64 length = ordered_overall_execution_count.size();
    for (size_t i = 0; i < length; i++)
    {
      /* Just keep the largest value*/
      uns64 ori_count = ordered_overall_execution_count[i].second;
      granu_overall_execution_count[strtoul((ordered_overall_execution_count[i].first).c_str(), NULL, 10)]= ori_count; //floor(i/(length/pow(2, UOP_ACCESS_COUNT_BIT)));
      // printf("%d, %d, %f, %f, %d, %d, %f\n", length, UOP_ACCESS_COUNT_BIT, pow(2, UOP_ACCESS_COUNT_BIT), length/pow(2, UOP_ACCESS_COUNT_BIT), i, ori_count, floor(i/(length/pow(2, UOP_ACCESS_COUNT_BIT))));
    }
    
    printf("Initializing footprint with size %u\n", granu_overall_execution_count.size());
    // for(auto it = granu_overall_execution_count.begin(); it != granu_overall_execution_count.end(); ++it)
    // {
    //   if (it->second>8000)
    //     std::cout << it->first << " " << it->second << endl;//" " << second_overall_execution_count[strtoul((it->first).c_str(), NULL, 10)] << endl;
    // }
...
uns64 find_entry_count(Addr addr){
  // return (overall_execution_count[addr]);
  // char temp[21];
  // sprintf(temp, "%z", addr);
  // uint64_t convert = (uint64_t) (granu_overall_execution_count[temp]);
  // printf("In find: %s %llu %d\n", temp, convert, granu_overall_execution_count[temp]);
  // return (convert);
  if (granu_overall_execution_count.count(addr)){
